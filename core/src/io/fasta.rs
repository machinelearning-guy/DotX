...
impl FastaParser {
    ...
    pub fn parse_reader<R: std::io::Read>(reader: R) -> Result<Vec<DotxSequence>> {
        let mut sequences = Vec::new();
    ...
    fn record_to_sequence(record: needletail::parser::SequenceRecord) -> Result<DotxSequence> {
        ...
        // Extract description if present (everything after the first whitespace in header)
        let description = if let Ok(full_header) = record.head() {
            let header_str = String::from_utf8_lossy(full_header);
            let parts: Vec<&str> = header_str.splitn(2, char::is_whitespace).collect();
            if parts.len() > 1 {
