...
impl PreviewAligner {
    ...
    pub fn preview_align(&self, ref_seq: &[u8], qry_seq: &[u8], k: u16, w: u16) -> Result<Vec<PafRecord>> {
        ...
        for chain in chains {
            if let Some(record) = self.chain_to_paf(&chain, ref_seq.len() as u64, qry_seq.len() as u64) {
    ...
    fn hash_kmer(&self, kmer: &[u8]) -> u64 {
        ...
        let mut hash = 0u64;
        for &base in kmer {
            hash = hash.wrapping_mul(4).wrapping_add(match base.to_ascii_uppercase() {
                b'A' => 0,
                b'C' => 1,
                b'G' => 2,
                b'T' => 3,
    ...
    fn seeds_compatible(&self, seed1: &Seed, seed2: &Seed) -> bool {
        ...
        ref_diff > 0 && qry_diff > 0 && 
        (ref_diff - qry_diff).abs() < 1000 // Allow some divergence
    }
