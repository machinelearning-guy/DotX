...
impl MummerParser {
    ...
    fn delta_to_anchor(
        ...
        let alignment_length = (ref_end - ref_start + 1).max(norm_query_end - norm_query_start + 1);
        let residue_matches = if alignment_length > errors {
            (alignment_length - errors) as u32
        } else {
            0
        };
...
mod tests {
    ...
    fn test_parse_coords_line() {
        ...
        assert_eq!(anchor.target_name, "ref_seq");
        assert_eq!(anchor.query_name, "query_seq");
        assert_eq!(anchor.target_start, 999); // 0-based
        assert_eq!(anchor.target_end, 2000);
        assert_eq!(anchor.query_start, 499); // 0-based
        assert_eq!(anchor.query_end, 1500);
    ...
    fn test_parse_coords_line_reverse() {
        ...
        assert_eq!(anchor.query_start, 499); // Normalized and 0-based
        assert_eq!(anchor.query_end, 1500);
    }
    ...
    fn test_parse_coords_reader() {
        ...
        assert_eq!(anchors[0].query_name, "query_seq");
        assert_eq!(anchors[1].query_name, "query_seq2");
