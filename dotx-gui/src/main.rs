            ...
            match FastaValidator::validate_file(&path) {
                Ok(validation) => {
                    if !validation.is_valid {
                        log::warn!("FASTA validation failed for {}: {:?}", path.display(), validation.errors);
                        // Still allow the file to be selected but warn user
                    } else {
                        log::info!("FASTA file validated: {} sequences, {} total bases", 
                                  validation.sequence_count, validation.total_length);
                    }
                }
                Err(e) => {
        ...
        egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("View", |ui| {
                    ui.checkbox(&mut self.ui_state.show_performance_panel, "Performance Panel");
                });
        ...
        egui::SidePanel::right("properties_panel")
            ...
            .show(ctx, |ui| {
                ...
                ui.collapsing("Display", |ui| {
                    ui.label("Color by:");
                    egui::ComboBox::from_id_source("color_by")
                    ...
                    egui::Grid::new("new_project_grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            egui::ComboBox::from_id_source("preset_combo")
                                ...
                                .show_ui(ui, |ui| {
                                    ui.selectable_value(&mut self.ui_state.selected_preset, AlignmentPreset::Bacterial, "Bacterial");
                                    ui.selectable_value(&mut self.ui_state.selected_preset, AlignmentPreset::PlantTE, "Plant TE");
                                    ui.selectable_value(&mut self.ui_state.selected_preset, AlignmentPreset::Mammal, "Mammal");
                                    ui.selectable_value(&mut self.ui_state.selected_preset, AlignmentPreset::Viral, "Viral");
                                    ui.selectable_value(&mut self.ui_state.selected_preset, AlignmentPreset::Custom, "Custom");
                                });
                            ui.end_row();
                        });
