...
impl FastaIndex {
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let mmap = unsafe {
            Mmap::map(&file)
                .map_err(|e| anyhow!("Failed to memory map FASTA file {}: {}", path.display(), e))?
        };
                ...
                b'\n' | b'\r' => {
                    if in_header {
                        ...
                        if start_idx < mmap.len() && end_idx <= mmap.len() && start_idx <= end_idx {
                            current_header =
                                String::from_utf8_lossy(&mmap[start_idx..end_idx]).to_string();
                        } else {
                            return Err(anyhow!("Invalid header bounds in FASTA file"));
                        }
                        in_header = false;
                        let _ = pos + 1; // sequence_start would be set here
    ...
    pub fn extract_sequence(
        ...
        let mmap = unsafe {
            Mmap::map(&file).map_err(|e| {
                anyhow!(
                    "Failed to memory map FASTA file {}: {}",
                    self.path.display(),
                    e
                )
            })?
...
impl FastaValidator {
    ...
    pub fn validate_file<P: AsRef<Path>>(path: P) -> Result<ValidationResult> {
        ...
        let mmap = unsafe {
            Mmap::map(&file)
                .map_err(|e| anyhow!("Failed to memory map file {}: {}", path.display(), e))?
        };
                ...
                _ => {
                    if in_header {
                        ...
                        match byte {
                            ...
                            | b'R' | b'Y' | b'S' | b'W' | b'K' | b'M' | b'r' | b'y' | b's'
                            | b'w' | b'k' | b'm' | b'B' | b'D' | b'H' | b'V' | b'b' | b'd'
                            | b'h' | b'v' | b' ' | b'\t' => {
                                if byte != b' ' && byte != b'\t' {
                                    current_seq_len += 1;
                                }
                            ...
                            _ => {
                                warnings.push(format!(
                                    "Invalid nucleotide '{}' at position {} (line {})",
