...
impl SamParser {
    ...
    fn record_to_anchor(
        ...
        // Reference name via header using reference_sequence_id index (SAM/BAM)
        let reference_name: String = match record.reference_sequence_id() {
            Ok(Some(rid)) => {
                // Header reference sequences preserve insertion order; map id -> name
                match header.reference_sequences().get_index(rid) {
                    Some((name, _rs)) => name.to_string(),
        ...
        // Get alignment start (SAM is 1-based). Convert to 0-based Position.
        let alignment_start: Position = match record.alignment_start() {
            Ok(Some(pos1b)) => {
                let v = pos1b.get();
                if v <= 0 { return Err(SamError::InvalidPosition(v)); }
                (v as u64) - 1
            }
            _ => 0,
        };
        ...
        // Get mapping quality
        let mapping_quality: Option<u8> = match record.mapping_quality() {
            Ok(Some(mq)) => {
                // MappingQuality is a newtype around u8; expose as u8
                #[allow(deprecated)]
                let v = mq.get();
                Some(v)
            }
        ...
        let reference_length: Position = header
            .reference_sequences()
            .get(&reference_name)
            .and_then(|rs| rs.length())
            .map(|len| len.get() as u64)
            .unwrap_or(0);
