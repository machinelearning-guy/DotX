...
impl MafParser {
    ...
    pub fn iter_blocks<P: AsRef<Path>>(path: P) -> Result<MafBlockIterator<BufReader<Box<dyn std::io::Read>>>> {
        use std::io::Read;
        let file = File::open(&path)?;
        let path_str = path.as_ref().to_string_lossy();
        
        let boxed: Box<dyn Read> = if path_str.ends_with(".gz") {
...
mod tests {
    ...
    fn test_block_to_anchors() {
        ...
        assert_eq!(anchor.q, "panTro1.chr6");
        assert_eq!(anchor.t, "hg18.chr7");
        assert_eq!(anchor.residue_matches, Some(13));
